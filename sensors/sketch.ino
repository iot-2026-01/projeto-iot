  ...
  void update_display() {
    switch (display_mode) {
      ...
      case 2:
        lcd.setCursor(0, 0);
        lcd.print(F("Eventos:"));
        if      (total_overload_events < 10)    { lcd.print(F("       ")); }
        else if (total_overload_events < 100)   { lcd.print(F("      ")); }
        else if (total_overload_events < 1000)  { lcd.print(F("     ")); }
        else if (total_overload_events < 10000) { lcd.print(F("    ")); }
        else                                    { lcd.print(F("   ")); }
        lcd.print(total_overload_events);

        lcd.setCursor(0, 1);
        lcd.print(F("Redistrib:"));
        if      (redistribution_count < 10)   { lcd.print(F("    ")); }
        else if (redistribution_count < 100)  { lcd.print(F("   ")); }
        else if (redistribution_count < 1000) { lcd.print(F("  ")); }
        else                                  { lcd.print(' '); }
